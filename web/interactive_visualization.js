...
function drawNode(node, x, y) {
    const g = d3.select('#circuit-container svg').append('g')
...
function drawConnection(connection) {
    function getEndpointPos(comp, term) {
        ...
        if (componentPositions[comp]) {
            let type = circuitData.components.find(c => c.id === comp)?.type || 'Resistor';
    ...
    d3.select('#circuit-container svg').append('line')
...
function updateConnections() {
    d3.select('#circuit-container svg').selectAll('.connection').remove();
