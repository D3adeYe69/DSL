...
class UnitConversionVisitor:
    ...
    def visit_analysis(self, analysis: Analysis):
        """Visit analysis simulations"""
        for sim in analysis.simulations:
            for param in sim.parameters.values():
                self.visit_expression(param)
        ...
        for endpoint in conn.endpoints:
            if isinstance(endpoint, Terminal):
                if endpoint.component_name not in self.component_names:
                    self.add_warning(f"Connection references undefined component: {endpoint.component_name}", conn)
...
class SubcircuitFlattener:
    ...
    def flatten_subcircuit_instance(self, instance: SubcircuitInstance) -> tuple[List[ComponentDeclaration], List[Connection]]:
...
class Interpreter:
    ...
    def build_connectivity(self, components: List[ComponentDeclaration]):
        ...
        for conn in self.program.connections:
            ...
            is_ground_connection = any(
                (isinstance(ep, str) and ep.lower() in ['ground', 'gnd', '0']) or
                (isinstance(ep, Node) and ep.is_ground)
            ...
            if is_ground_connection:
                ...
                if node_name and node_name in self.nodes:
                    ...
                    string_endpoints = [ep for ep in conn.endpoints if isinstance(ep, str) and ep not in ['ground', 'gnd', '0']]
                    existing_node = None
            ...
            for endpoint in conn.endpoints:
                if isinstance(endpoint, Terminal):
                    ...
                    self.terminal_connections[endpoint.component_name][endpoint.terminal_name] = target_node
                elif isinstance(endpoint, str) and endpoint not in ['ground', 'gnd', '0']:
                    # Named node
    ...
    def generate_netlist(self) -> List[str]:
        ...
        if not validator.validate_program(self.program):
            self.errors.extend(validator.errors)
            self.warnings.extend(validator.warnings)
            return lines  # Don't proceed if validation failed
        
        self.warnings.extend(validator.warnings)
        
        # Step 2: Unit conversion
        unit_converter = UnitConversionVisitor()
        unit_converter.visit_program(self.program)
        
        # Step 3: Flatten subcircuits
        subcircuit_map = {subckt.name: subckt for subckt in self.program.subcircuits}
        flattener = SubcircuitFlattener(subcircuit_map)
        flattened_program = flattener.flatten_program(self.program)
        
        # Update program to flattened version
        self.program = flattened_program
        
        # Step 4: Process variables
        self.process_variables()
        
        # Step 5: Expand macros and loops
        expanded_components = self.expand_macros()
        
        # Step 6: Build connectivity
        self.build_connectivity(expanded_components)
        
        # Step 7: Generate component lines
        for comp in expanded_components:
            if isinstance(comp, ComponentDeclaration):
                try:
                    line = self._format_component(comp)
                    if line:
                        lines.append(line)
                except Exception as e:
                    self.add_error(f"Error formatting component {comp.instance_name}: {e}", comp)
        
        # Step 8: Generate simulation commands
        for analysis in self.program.analyses:
            for sim in analysis.simulations:
                try:
                    sim_line = self._format_simulation(sim)
                    if sim_line:
                        lines.append(sim_line)
                except Exception as e:
                    self.add_error(f"Error formatting simulation: {e}", sim)
        
        return lines
    
    def _format_component(self, comp: ComponentDeclaration) -> Optional[str]:
